...
export default function CategoriesPage() {
  ...
  // Charger catégories + transactions
  useEffect(() => {
    if (user) {
      loadCategories();
      loadTransactions();
  ...
  const handleDelete = async (category: Category) => {
    try {
      ...
      const isUsed = transactions.some(
        (t: { category: string }) => t.category === category.name
      );

      if (isUsed) {
        if (
          !confirm(
            'Cette catégorie est utilisée dans des transactions. Êtes-vous sûr de vouloir la supprimer ?'
          )
        ) {
          return;
        }
  ...
  // Compter le nombre de transactions associées à une catégorie
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Gérez vos catégories de dépenses et revenus
              </p>
            </div>
